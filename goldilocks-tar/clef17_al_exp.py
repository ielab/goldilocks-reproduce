...
from al_record import DefaultMetrics, HeuristicsStoppingInfo # change from al_record to clef_record for saving runs
...
wandb.init(
    ...
    project="goldilocks-reproduce",

    # track hyperparameters and run metadata
    config={
        "model": "bert-base-cased",
        "logits or probability": "probability",
        "dataset": "clef17",
        "category": "test",
        "frozen": "True",
        "pre-train epoch": 2,
...
def main(args):
    ...
    for topic, qsname in product(args.topic, args.sampling_strategy):
        tag = f"clef17_test_{topic}_{args.iseed}_{qsname}"
        ...
        model = TransformerModel(torch_model, collator,
                                 ...
                                 per_device_train_batch_size=100,  # 26 fixed for rtx GPU, 50, 50, 200
                                 per_device_eval_batch_size=1000,  # 600 fixed for rtx GPU, 1000, 2000, 700
        ...
        for i in range(it_start, args.niter):
            ...
            if i == 0:
                ...
                # Q: logits = unnormalized probabilities, interpreted as uncertainty for al?
            ...
            # logits = model.predict_proba(full_ds.get_labeled_features())
            # postrun
            logits = model.predict_real(full_ds.get_labeled_features())
            ...
            wandb.log({"R-P": metrics[-1]['R-P'], "WSS85": metrics[-1]['WSS@85'], "WSS@95": metrics[-1]['WSS@95'],
                       "uni-cost": cost[-1][(0.8, 'uni-cost')], "exp-train": cost[-1][(0.8, 'exp-train')]})
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_path', type=str, default='bert-base-cased') # /scratch/project/ai_systematic_review/goldilocks-reproduce/mlm-finetune/clef/2018/bert-base_clef_2018_test_CD012599_ep10
