...
class DefaultMetrics(ActiveLearningRecord):
    ...
    def evaluate(self, score):
        df = pd.DataFrame({"gold": self.gold, "score": score, "known": self.known})
        df = df.assign(adjscore=((df.gold - 0.5) * np.inf * df.known).fillna(0) + df.score).sort_values('adjscore',
                                                                               ascending=False)
...
class HeuristicsStoppingInfo(ActiveLearningRecord):
    ...
    def evaluate(self, score):
        df = pd.DataFrame({'gold': self.gold.astype(bool), 'score': score, 'known': self.known})
        df = df.assign(adjscore=((df.gold - 0.5) * np.inf * df.known).fillna(0) + df.score).sort_values('adjscore',
                                                                                                        ascending=False)
        ...
        numbers[(-1, 'scores-info', 'estprob-bivar-unknown')] = (est_probs * (1 - est_probs))[
            ~self.known].sum() if has_unknown else None
        numbers[(-1, 'scores-info', 'estprob-median-unknown')] = np.median(unknown_est_probs) if has_unknown else None

        numbers[(-1, 'scores-info', 'max-uncertain-unknown')] = np.abs(
            unknown_est_probs - 0.5).max() if has_unknown else None
        numbers[(-1, 'scores-info', 'max-score-unknown')] = score[~self.known].max() if has_unknown else None
        numbers[(-1, 'scores-info', 'expected-error-unknown')] = (
                    unknown_est_probs * (1 - unknown_est_probs)).sum() if has_unknown else None
...
class WithControlSet(object):
    ...
    def evaluate(self, score):
        ...
        df = df.assign(adjscore=((df.gold - 0.5) * np.inf * df.known).fillna(0) + df.score).sort_values(
            ['adjscore', 'score'], ascending=False)
