...
def main(args):
    ...
    for cate, qsname in product(args.category, args.sampling_strategy):
        ...
        if (args.output_path / f"{tag}.results.pkl").exists():
            if args.resume_runs:
                ...
                try:
                    ...
                    probabilities = infos[-1]['probabilities'] #llogits
        ...
        model = TransformerModel(torch_model, collator,
                                 ...
                                 per_device_train_batch_size=100,  # 26 fixed for original authors' rtx GPU
                                 per_device_eval_batch_size=1000,  # 600 fixed for original authors' rtx GPU
        ...
        for i in range(it_start, args.niter):
            ...
            if i == 0:
                ...
                # original authors directly use logits throughout the exp, we use probabilities for active learning
                ask_id = qs.make_query(n_ask=args.batch_size, retrain=False, dvalue=probabilities) # logits
            ...
            _logger.info(f"it={i} done -- R-P={metrics[-1]['R-P']}, DFR0.8={metrics[-1]['DFR@0.8']}, "  
...
if __name__ == '__main__':
    ...
    parser.add_argument('--target_recall', type=float, nargs='+', default=[0.95, 0.9, 0.85, 0.8, 0.7]) # [0.95, 0.9, 0.8, 0.7, 0.6]

    parser.add_argument('--model_type', choices=['Bert'], default='Bert')
    parser.add_argument('--model_path', type=str, default='bert-base-cased')
    parser.add_argument('--cached_dataset', type=Path, default='./cache_new/rcv1-20sub_org_bert-base.512.pkl.gz')
